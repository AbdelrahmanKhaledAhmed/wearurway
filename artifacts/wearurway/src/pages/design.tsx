...
export default function Design() {
  ...
  // Same pinch handlers wired to the mobile clip area.
  useEffect(() => {
    const el = mobileClipAreaRef.current;
    if (!el) return;
    el.addEventListener("touchstart", onTouchStart, { passive: false });
    el.addEventListener("touchmove", onTouchMove, { passive: false });
    el.addEventListener("touchend", onTouchEnd);
  ...
  return (
        ...
        <div className="w-72 border-l border-border flex flex-col shrink-0 overflow-hidden">
          ...
          <div className="p-6 border-b border-border space-y-3">
            ...
            <div className="space-y-1.5 pt-1">
              <p className="text-xs text-muted-foreground uppercase tracking-widest">Size of T-Shirt</p>
              <div className="flex gap-2">
                <button
    ...
    {/* ════════════════════════════════════════════
        ...
        ════════════════════════════════════════════ */}
    <div className="flex flex-col md:hidden min-h-screen bg-background no-scrollbar">
      ...
      {/* ── Mockup view size ── */}
      <div className="px-4 py-4 border-b border-border">
        <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mb-2">T-Shirt Size</p>
        <div className="flex gap-2">
          <button
            onClick={() => setViewZoom(z => Math.max(VIEW_ZOOM_MIN, +(z - VIEW_ZOOM_STEP).toFixed(2)))}
            disabled={viewZoom <= VIEW_ZOOM_MIN + 1e-3}
      ...
      <button
        ...
        className="fixed top-20 right-4 z-40 flex items-center gap-2 px-4 py-3 border border-border bg-background shadow-lg text-xs font-bold uppercase tracking-widest hover:border-foreground transition-colors"
      >
