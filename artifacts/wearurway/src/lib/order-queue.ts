 ...
 */
export function generateOrderId(suffix?: string): string {
  const part = () => Math.random().toString(36).slice(2, 6).toUpperCase();
  const base = `WW-${part()}${part()}`;
...
export async function saveSpecQueuedOrder(
  ...
  const orderId = generateOrderId(args.source);
  const now = new Date().toISOString();
  const record: QueuedOrderSpecRecord = {
    id: queueId,
    kind: "spec",
    orderId,
    customer: { ...args.customer, source: args.source },
