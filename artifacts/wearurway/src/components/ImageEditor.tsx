...
export default function ImageEditor({ file, onConfirm, onCancel, qualityScale=1 }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex flex-col" style={{backgroundColor:"#141414"}}>
      ...
      {showBgOnboarding && (         <div           className="fixed inset-0 z-[110] flex items-center justify-center p-4"           style={{ backgroundColor: "rgba(0,0,0,0.82)", backdropFilter: "blur(8px)" }}         >           <div             className="w-full max-w-sm rounded-3xl p-7 text-center"             style={{               background: "linear-gradient(135deg,rgba(20,10,40,0.99),rgba(10,5,25,0.99))",               border: "1px solid rgba(168,85,247,0.5)",               boxShadow: "0 24px 64px rgba(124,58,237,0.5)",             }}           >             {/* Animated wand icon */}             <div className="relative w-16 h-16 mx-auto mb-5 flex items-center justify-center rounded-2xl"               style={{ background: "linear-gradient(135deg,rgba(168,85,247,0.25),rgba(124,58,237,0.15))", border: "1px solid rgba(168,85,247,0.35)" }}>               <div className="absolute inset-0 rounded-2xl animate-ping opacity-20"                 style={{ backgroundColor: "rgba(168,85,247,0.5)" }} />               <svg viewBox="0 0 24 24" fill="none" stroke="#a855f7" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" style={{ width: 30, height: 30 }}>                 <path d="M15 4V2"/><path d="M15 16v-2"/><path d="M8 9h2"/><path d="M20 9h2"/>                 <path d="M17.8 11.8 19 13"/><path d="M15 9h.01"/><path d="M17.8 6.2 19 5"/>                 <path d="m3 21 9-9"/><path d="M12.2 6.2 11 5"/>               </svg>             </div>              <p className="text-[10px] font-black uppercase tracking-[0.3em] mb-2" style={{ color: "#c48cff" }}>               Remove Background             </p>             <h2 className="text-[18px] font-black text-white mb-3 leading-snug">               Tap the background<br/>to remove it             </h2>             <p className="text-[12px] leading-relaxed mb-2" style={{ color: "rgba(255,255,255,0.6)" }}>               Magic Select is already active. Just tap anywhere on the background of your image — it will be removed instantly.             </p>              {/* Visual demo hint */}             <div className="flex items-center justify-center gap-3 my-4 px-4 py-3 rounded-2xl"               style={{ backgroundColor: "rgba(168,85,247,0.1)", border: "1px solid rgba(168,85,247,0.2)" }}>               <div className="flex flex-col items-center gap-1">                 <div className="w-10 h-10 rounded-xl flex items-center justify-center text-lg"                   style={{ backgroundColor: "rgba(255,255,255,0.07)" }}>👆</div>                 <p className="text-[9px] uppercase tracking-wider" style={{ color: "rgba(196,140,255,0.7)" }}>Tap bg</p>               </div>               <svg viewBox="0 0 24 24" fill="none" stroke="rgba(168,85,247,0.5)" strokeWidth="2" style={{ width: 18, height: 18 }}>                 <path d="M5 12h14M12 5l7 7-7 7"/>               </svg>               <div className="flex flex-col items-center gap-1">                 <div className="w-10 h-10 rounded-xl flex items-center justify-center text-lg"                   style={{ backgroundColor: "rgba(255,255,255,0.07)" }}>✨</div>                 <p className="text-[9px] uppercase tracking-wider" style={{ color: "rgba(196,140,255,0.7)" }}>Gone!</p>               </div>             </div>              <p className="text-[10px] mb-5" style={{ color: "rgba(255,255,255,0.3)" }}>               Tap multiple times to remove more areas. Use Undo (Ctrl+Z) to go back.             </p>              <button               onClick={() => setShowBgOnboarding(false)}               className="w-full py-4 rounded-2xl text-[13px] font-black uppercase tracking-widest transition-all hover:scale-[1.02] active:scale-[0.98]"               style={{ background: "linear-gradient(135deg,#a855f7,#7c3aed)", color: "#fff", boxShadow: "0 8px 24px rgba(168,85,247,0.4)" }}             >               Got it — Let's go! ✦             </button>           </div>         </div>       )}        {showHelpWizard && (
        <div
