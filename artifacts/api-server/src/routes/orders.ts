...
function generateOrderId(suffix?: string): string {
  const base = `WW-${Math.floor(10000 + Math.random() * 90000)}`;
...
router.post("/create-order", (req, res) => {
  ...
  const source = typeof body.source === "string" ? body.source.toLowerCase() : undefined;
  const suffix = source === "meta" ? "META" : undefined;

  const orderId =
    body.orderId && CLIENT_ORDER_ID_RE.test(body.orderId)
      ? body.orderId
      : generateOrderId(suffix);
