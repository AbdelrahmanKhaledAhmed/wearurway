...
async function sendTelegramMessage(orderId: string, payload: NotificationPayload): Promise<void> {
  ...
  "",
  "<b>CUSTOMER</b>",
  `${pad("Name:", 10)}${payload.name ?? "-"}`,
  `${pad("Phone:", 10)}${payload.phone ?? "-"}`,
  `${pad("Address:", 10)}${payload.address ?? "-"}`,
  "",
  "<b>PRODUCT</b>",
  `${pad("Product:", 10)}${payload.product ?? "-"}`,
  `${pad("Fit:", 10)}${payload.fit ?? "-"}`,
  `${pad("Color:", 10)}${payload.color ?? "-"}`,
  `${pad("Size:", 10)}${sizeDetails}`,
  "",
  "<b>PRICING</b>",
  `${pad("T-shirt:", 10)}${formatMoney(productPrice)}`,
  `${pad("Shipping:", 10)}${formatMoney(shippingPrice)}`,
  `${pad("Total:", 10)}${formatMoney(payload.total)}`,
