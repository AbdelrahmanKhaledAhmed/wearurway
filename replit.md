...
- `POST /api/create-order` creates a `WW-xxxxx` order ID, sends the order summary to Telegram via `sendMessage`, then sends every high-resolution PNG generated by the same Export Design pipeline sequentially as Telegram documents via `sendDocument`. Checkout previews remain display-only fallbacks.
- `POST /api/create-order` now returns the `WW-xxxxx` order ID immediately, then sends the Telegram order summary, InstaPay proof screenshot when applicable, and print/mockup documents asynchronously. Telegram messages include payment method plus product, shipping, and total price breakdown.
