...
- Design editor supports cursor-centered mouse-wheel zoom on selected image layers, free layer dragging, and an edit-image modal with deep cursor-centered zoom, Move/pan tool, precise circular brush erase, and automatic transparent-edge trimming before upload plus on image load, brush erase, and fill removal so layer bounds shrink to visible pixels.
